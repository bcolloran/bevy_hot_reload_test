...
// choose a random color, discretizing to 4 possible colors
#[no_mangle]
...
pub fn player_shooting_system(
    ...
    // world.
    let keyboard_input = world.remove_resource::<Input<KeyCode>>().unwrap();
    ...
    if keyboard_input.just_pressed(KeyCode::Space) {
        ...
        println!("shooting {:?}", color.as_rgba_u8().to_ascii_uppercase());
        if let Ok(tfm) = query.get_single(world) {
            world
                .spawn((
                    SpriteBundle {
                        transform: *tfm,
